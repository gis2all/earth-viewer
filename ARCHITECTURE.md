# ARCHITECTURE.md — Earth Viewer 目标架构

> 本文是 **refactor 分支的重构路线图**：描述目标架构，而非当前现状。当前代码与本文不一致是预期——迁移按 §5 路线图推进，每步结束时行为不变、测试全绿。架构决策落地前，以此文件为准；与代码冲突时，先确认哪一方代表最新已确认设计，再同步另一方。
...
- **验收底线**：每一步重构行为不变、338 单测全绿、覆盖率门禁不降、E2E 通过。

**★ 不可违反规则**：
1. 依赖只能外层→内层，反向禁止；`Domain` 零依赖（纯 TS 类型与规则，不 import Cesium/React/fetch）。
2. 全项目除 `Infra` 层外，不得出现 `Cesium.` / MapLibre 直接引用。
...
代码整体质量良好（模块划分清晰、338 单测、覆盖率 90%+），但三个热点文件承担了过多职责：
...
| 目标模块 | 设计模式 | 现有雏形 | 迁移动作 |
|---|---|---|---|
| `controller/CameraController` | Command + Observer | `cameraApi.ts` | 收编滚轮/双击/自动环绕，输出相机状态流 |
| `controller/LayerController` | State Machine + Domain Events | store.added + layerErrors（分散） | 收敛图层生命周期为唯一状态机 |
| `service/ArcGISRepository` | Repository + Middleware | webmap.ts fetch + LayerPanel 预检缓存 | 收口 search/元数据/data/探测，内挂超时/429/缓存 |
| `domain/LayerAdapter` 注册表 | Strategy / Plugin | `is*Layer` + `providerFor*` | 统一为接口 + 注册表，新增类型只加 adapter |
| `service/LayerLoader` | Pipeline | 散在 GlobeViewer 内 | 预检→取数→转换→上球串成管线 |
| `service/ViewportService` | Layer/LayerView 单向流 | `viewport/` 目录 | 生命周期交给控制器，保留现管线 |
| `service/LayerScheduler` | Scheduler | `renderQueue` + LRU | 视口优先级 + 串行队列 + 取消统一 |
| `infra/CesiumFacade` | Facade | GlobeViewer.tsx（过胖） | 对外只暴露业务语义，项目内不散见 `Cesium.` |
| `infra/MapLibrePool` | 资源池 | maplibreImagery.ts | 样式解析与渲染池分离 |
| `state/Store` | 状态容器 | store.ts | 只存全局态，图层运行时状态迁入控制器 |
...
src/
  ...
  globe/        # 收敛为 Cesium 相关实现：facade/ adapters/ render/
  controller/   # CameraController / LayerController / EffectsController
  service/      # repository / loader / viewport / scheduler / budget
  domain/       # types / registry / state-machine / policy（纯 TS）
  state/        # zustand store（持久化）
  infra/        # cache / http（中间件）/ worker
```
...
| 现状 | 目标 |
|---|---|
| `src/globe/GlobeViewer.tsx`（913 行） | `infra/CesiumFacade` + `controller/CameraController` + `controller/LayerController` |
| `src/app/LayerPanel.tsx`（715 行） | 瘦组件 + `app/icons.tsx` + `service/ArcGISRepository` 相关片段 |
| `src/globe/webmap.ts` | `domain/registry` + `service/repository` + `infra/facade` 适配器 |
| `src/globe/maplibreImagery.ts` | `infra/MapLibrePool`（样式解析与渲染池分离） |
| `src/globe/cameraApi.ts` | `controller/CameraController` |
| `src/globe/viewport/` | `service/ViewportService`（保留现管线） |
| `src/globe/loadSafety.ts` | `domain/policy` + `service/BudgetManager` |
...
- `npm run lint`
- `npm run test:coverage`：338 单测，statements/lines ≥ 90%
- `npm run build`
- `npm run test:e2e`：28 项
- `rg` 检查：`src/` 中除 `infra/` 与 `globe/facade/` 外无 `Cesium.` 直接引用；无反向 import（内层不得 import 外层）
- `git diff --check`
